...
PATH_MDPI+='/src/main/res/drawable-mdpi'
...
do
  NEW_NAME="img_$f"
  TMP="$PATH_MDPI/$NEW_NAME"
  echo "$TMP"
  mv "$f" "$PATH_MDPI/$NEW_NAME"
done
...
do
  SUFFIX="@1.5x.png"
  BASE_NAME=${f%"$SUFFIX"}
  NEW_NAME="img_${BASE_NAME}.png"
  DEST_PATH="$PATH_HDPI/$NEW_NAME"
  echo "$DEST_PATH"
  mv "$f" "$DEST_PATH"
done
...
do
  ...
  BASE_NAME=${f%"$SUFFIX"}
  NEW_NAME="img_${BASE_NAME}.png"
  DEST_PATH="$PATH_XXHDPI/$NEW_NAME"
  echo "$DEST_PATH"
  mv "$f" "$DEST_PATH"
done
...
do
  ...
  BASE_NAME=${f%"$SUFFIX"}
  NEW_NAME="img_${BASE_NAME}.png"
  DEST_PATH="$PATH_XXXHDPI/$NEW_NAME"
  echo "$DEST_PATH"
  mv "$f" "$DEST_PATH"
done