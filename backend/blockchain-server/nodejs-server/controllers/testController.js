...
const { web3, systemContract, coinContract, NFTContract, account, testAccount } = require('../utils/web3Helper');
...
exports.startAuctionTest = async (req, res) => {
  try {
    ...
    // 가스 가격 및 가스 한도 추정
    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await systemContract.methods.startAuction(tokenId, amountInSmallestUnitTrimmed).estimateGas({ from: account.address });
    const nonce = await web3.eth.getTransactionCount(account.address, 'pending');
...
// 옥션 취소
exports.cancleAuctionTest = async (req, res) => {
  try {
    const address = req.body.address;
    const tokenId = req.body.tokenId;
    ...
    // 가스 가격 및 가스 한도 추정
    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await systemContract.methods.cancelAuction(tokenId).estimateGas({ from: account.address });
    const nonce = await web3.eth.getTransactionCount(account.address, 'pending');
...
// 마켓 생성
exports.createMarketTest = async (req, res) => {
  try {
    const address = req.body.address;
    const tokenId = req.body.tokenId;
    const price = req.body.price;
    ...
    // 가스 가격 및 가스 한도 추정
    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await systemContract.methods.createMarket(tokenId, amountInSmallestUnitTrimmed).estimateGas({ from: account.address });
    const nonce = await web3.eth.getTransactionCount(account.address, 'pending');
...
// 마켓 취소
exports.cancleMarketTest = async (req, res) => {
  try {
    const address = req.body.address;
    const tokenId = req.body.tokenId;
    ...
    // 가스 가격 및 가스 한도 추정
    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await systemContract.methods.cancelMarket(tokenId).estimateGas({ from: account.address });
    const nonce = await web3.eth.getTransactionCount(account.address, 'pending');
...
// 마켓 구매
exports.buyMarketTest = async (req, res) => {
  try {
    const address = req.body.address;
    const tokenId = req.body.tokenId;
    ...
    // 가스 가격 및 가스 한도 추정
    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await systemContract.methods.buyMarket(tokenId).estimateGas({ from: testAccount.address });
    const nonce = await web3.eth.getTransactionCount(testAccount.address, 'pending');
